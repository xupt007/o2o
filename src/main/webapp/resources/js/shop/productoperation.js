				...
				.getJSON(
						...
						function(data) {
							if (data.success) {
								...
								var optionSelected = product.productCategory.productCategroyId;
								// 生成前端的HTML商品类别列表，并默认选择编辑前的商品类别
								optionArr
										.map(function(item, index) {
											var isSelect = optionSelected === item.productCategroyId ? 'selected'
													: '';
											optionHtml += '<option data-value="'
													+ item.productCategroyId
													+ '"'
													+ isSelect
													+ '>'
	...
	$('#submit').click(
			...
			function() {
				...
				product.productCategory = {
					productCategroyId : $('#category').find('option').not(
							function() {
