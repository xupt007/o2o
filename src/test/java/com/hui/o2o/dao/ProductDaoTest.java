...
public class ProductDaoTest extends BaseTest{
	...
	public void testBQueryProductList() throws Exception {
		...
		//分页
		List<Product> productList = productDao.queryProductList(productC, 0, 3);
		assertEquals(3, productList.size());
		//查询名称为测试的商品总数
		int count = productDao.queryProductCount(productC);
		assertEquals(11, count);
		//模糊查询
		productC.setProductName("测试");
		productDao.queryProductList(productC, 0, 3);
		assertEquals(6, productList.size());
		count = productDao.queryProductCount(productC);
	...
	public void testEUpdateProductCategoryTonull() {
		int effe = productDao.updateProductCategoryTonull(7L);
		assertEquals(1, effe);
	}
}
